...
class TestAPICreation(BaseCase):
    ...
    def test_create_account(self, api_client):
        ...
        assert response_data["password"] == data["password"], f'There isn`t the {data["username"]} in database'
        assert response_data["email"] == data["email"], f'There isn`t the {data["username"]} in database'
        assert response_data["access"] == '1', f'There isn`t the {data["username"]} in database'
